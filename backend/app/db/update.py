...
def update_player_age(db, heap_date):
    ...
    for player_id, birth_date in rows:
        ...
        try:
            ...
            if len(batch) >= 500:
                db.executemany(f"UPDATE players SET age = ? WHERE player_id = ?", batch)
                db.commit()
                batch.clear()

        except ValueError:
            print(
                f"Skipping invalid birth_date format for player_id {player_id}: {birth_date}"
            )
...
def run_migration_short(heap_date, db):
    with current_app.open_resource(
        os.path.join("db", "sql_scripts", "migration", "migration_short.sql"), "r"
    ) as f:
...
def fetch_projection_inputs(heap_date, db):
    with current_app.open_resource(
        os.path.join("db", "sql_scripts", "migration", "get_projection_inputs.sql"), "r"
    ) as f:
