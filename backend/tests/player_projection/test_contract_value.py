...
def test_signed_contract_years_use_actual_salary():
    contract = make_contract(current_year=1, years=2, salary0=1_000_000, salary1=2_000_000)
    result = calculate_surplus_value(
...
def test_age_decline_applies_only_after_32():
    result = calculate_surplus_value(
        base_war=4.0, current_age=30, mlb_service_years=6, contract=make_contract(1, 6)
    )
...
def test_negative_surplus_is_not_floored():
    contract = make_contract(current_year=1, years=1, salary0=50_000_000)
    result = calculate_surplus_value(
